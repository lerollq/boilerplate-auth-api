...
declare global {
  namespace Express {
     interface Request {
       user?: boilerplateAuthApi.UserPayload,
     }
   }
 }
...
declare namespace boilerplateAuthApi  {
  ...
  interface JWTPayload extends UserPayload, RegisteredClaim {}
...
const boilerplateAuthApi = (app:Application, router:Router, options: boilerplateAuthApi.Options) => {
  ...
  const verify = (requiredRole: string[]) => async (req:Request, res: Response, next: NextFunction) => {
    try {
      ...
      const decoded = <boilerplateAuthApi.JWTPayload>jwtVerify(token, options.jwtSecret, options.jwtOptions);
